...
type Demuxer struct {
	mutex sync.RWMutex
	saver map[FullFlow]*saver.Saver
...
func (d *Demuxer) GetSaver(ctx context.Context, flow FullFlow) *saver.Saver {
	// Quickly read the struct and (usually) retrieve the Saver found there.
	d.mutex.RLock()
	s, ok := d.saver[flow]
	d.mutex.RUnlock()

	// The common case is to not need to do this block. This function gets
	// called for every packet, but this block of code only needs to execute
	// when a new flow appears. So we make the uncommon case slower in an effort
	// to unblock the common case. We also have to double-check the condition
	// after we re-acquire the read lock as a write lock.
	if !ok {
		...
		if !ok {
			d.saver[flow] = saver.StartNew(ctx, d.anon, d.dataDir, d.maxDuration)
		}
		// Release the write lock.
		d.mutex.Unlock()
	}

	// Whether it was retrieved or created, return the Saver.
	return s
}
...
// CapturePackets captures the packets and hands them off to the appropriate Saver.
...
func New(anon anonymize.IPAnonymizer, dataDir string, maxFlowDuration time.Duration) *Demuxer {
	return &Demuxer{
		saver: make(map[FullFlow]*saver.Saver),
